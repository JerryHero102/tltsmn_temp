...
import { Users, Receipt, LogOut, PlusCircle, Shield, Menu, X, User } from 'lucide-react';
      ...
      >
        ...
        <div>
          <div className="p-6 border-b border-emerald-800/80 flex items-center space-x-3">
            ...
            <div>
              <h1 className="font-extrabold text-lg tracking-tight text-white leading-tight">TLTSMN</h1>
          ...
          <nav className="p-4 space-y-2">
            ...
            >
              <Users className={`w-5 h-5 ${activeTab === 'students' ? 'text-emerald-300' : 'text-emerald-200'}`} />
              <span>{isAdmin ? 'Thông tin học viên' : 'Thông tin cá nhân'}</span>
            </button>
            ...
            >
              <Receipt className={`w-5 h-5 ${activeTab === 'tuition' ? 'text-emerald-300' : 'text-emerald-200'}`} />
              ...
              <div className="flex items-center gap-1 mt-0.5">
                <span
                  className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold uppercase ${
                    isAdmin ? 'bg-amber-400 text-amber-950' : 'bg-emerald-300 text-emerald-950'
                  }`}
                >
